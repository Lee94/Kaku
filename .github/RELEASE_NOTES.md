...
4. **Tool Sandbox Audit**: File-access policy ships with thorough symlink-escape and credential-path tests, and now blocks credential file names (`.env`, `*.pem`, `*.key`, SSH private keys) in any directory while still allowing committed `.env.example` templates.
...
4. **工具沙箱审计**：文件访问策略补充了软链逃逸与凭证路径的完整测试，并新增按文件名拦截凭证文件（`.env`、`*.pem`、`*.key`、SSH 私钥），任意目录均生效，同时放行 `.env.example` 这类示例模板。
